...
outer_re = re.compile(r'^([\w ]+) bags contain')
inner_re = re.compile(r'\d+ ([\w ]+) bags?[,.]')

def parse_rules():
    r = {}
    for rule in input2.split('\n'):
        if m := outer_re.search(rule):
            r[m.group(1)] = tuple(inner_re.findall(rule))
    return r
...
def part1_michi(color):
    ...
    while new > 0:
        ...
        for candidate in candidates:
            new += getOuterBag(candidate)
    return new

    
rules = parse_rules()
pprint.pprint(rules)
print(part1_michi(COLOR))
